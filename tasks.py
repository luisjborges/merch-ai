...
def create_tasks(agents, product_category, product_details):
    task1 = Task(
        description=dedent(f"""\
            Present category options and collect initial order details for category: {product_category}.
            Product details: {product_details}."""),
        expected_output="Order details captured and logged",
        agent=agents.order_intake_agent()
    )

    task2 = Task(
        description=dedent(f"""\
            Verify order details for category: {product_category} and initiate production.
            Check inventory levels: {500}.
        	Product details: {product_details}."""),
        expected_output="Order verified and production initiated",
        agent=agents.order_processing_agent()
    )

    task3 = Task(
        description=dedent(f"""\
            Gather customization preferences and confirm all specifications for category: {product_category}.
            Product details: {product_details}."""),
        expected_output="Customization details collected and confirmed",
        agent=agents.customization_agent()
    )

    task4 = Task(
        description=dedent(f"""\
            Coordinate production, communicate with clients, and ensure timely delivery for category: {product_category}.
            Product details: {product_details}."""),
        expected_output="Client updated and order delivered",
        agent=agents.coordination_agent()
    )
    ...
    # task2 = Task(
    #     description=dedent(f"""\
    #         Verify order details for category: {product_category}, check inventory, and initiate production.
    #         Product details: {product_details}."""),
    #     expected_output="Order verified and production initiated",
    #     agent=agents.order_processing_agent()
    # )
