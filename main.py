...
from agents import MerchandiseOrderAgents
...
# Create Agents
order_intake_agent = agents.order_intake_agent()
order_processing_agent = agents.order_processing_agent()
customization_agent = agents.customization_agent()
coordination_agent = agents.coordination_agent()

# Create Tasks
tasks = create_tasks(agents, product_category, product_details)

# Create Crew responsible for order processing
order_crew = Crew(
    agents=[order_intake_agent, order_processing_agent, customization_agent, coordination_agent],
    tasks=tasks,
    verbose=True
)

# Execute the tasks
result = order_crew.kickoff()

# Print results
print("\n\n########################")
print("## Order Process Result")
print("########################\n")
print(result)
